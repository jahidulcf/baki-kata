...
const UIState = {
    ...
    switchScreen(screenName) {
        ...
        // Update nav buttons
        document.querySelectorAll('.nav-btn').forEach(btn => btn.classList.remove('text-gray-900', 'border-gray-900'));
        document.querySelectorAll('.nav-btn').forEach(btn => btn.classList.add('text-gray-500', 'border-transparent'));
...
const ReportScreen = {
    render() {
        ...
        if (customers.length === 0) {
            container.innerHTML = '<div class="text-sm text-gray-500 text-center py-8">কোনো গ্রাহক নেই</div>';
            return;
        }

        const reportHTML = customers.map(customer => {
            const balance = transactions
                .filter(t => t.customerId === customer.id)
                .reduce((sum, t) => {
                    return sum + (t.type === 'sale' ? t.amount : -t.amount);
                ...
                <div class="bg-white rounded-lg border border-gray-200 p-4">
                    <div class="flex items-center justify-between mb-3">
                        <div>
                            ...
                            <p class="text-xs text-gray-600">${customer.phone || 'ফোন নেই'}</p>
                        </div>
                        <button class="generate-pdf-btn px-3 py-1 bg-blue-600 text-white rounded text-xs font-semibold" data-customer-id="${customer.id}">
                            পিডিএফ তৈরি করুন
                        </button>
                    </div>
